...
TARGET = '../target/main/Circom/parser' if os.name == 'posix' else os.path.normpath(
    '../target/')
locpath = ['./main/Circom/parser/',
           './main/Circom/astgen/', './main/Circom/utils/']
for p in locpath:
...
def main(argv):
    ...
    if len(argv) < 1:
        ...
        if not './main/Circom/parser/' in sys.path:
            sys.path.append('./main/Circom/parser/')
        ...
        if len(argv) < 2:
            ...
            test(suite)
        elif argv[1] == 'ASTSuite':
            from ASTSuite import ASTSuite
            suite = unittest.makeSuite(ASTSuite)
            test(suite)
        else:
            printUsage()
    else:
...
def printUsage():
    ...
    print("python3 run.py test ASTSuite")
