...
class ASTGeneration(CircomVisitor):
    ...
    def visitProgram(self, ctx: CircomParser.ProgramContext):
        ...
        for definition in definitions:
            if isinstance(definition, Template) and definition.is_custom_gate:
                ...
                break
        return Program(FileLocation(ctx.start, ctx.stop), compile_version, custom_gates, custom_gates_declare, includes, definitions, main_component)
    ...
    def visitInclude_definition(self, ctx: CircomParser.Include_definitionContext):
        return Include(FileLocation(ctx.start, ctx.stop), ctx.STRING().getText())
    ...
    def visitMain_component(self, ctx: CircomParser.Main_componentContext):
        return MainComponent(FileLocation(ctx.start, ctx.stop), self.visit(ctx.public_list()), self.visit(ctx.expression()))
    ...
    def visitFunction_definition(self, ctx: CircomParser.Function_definitionContext):
        return Function(FileLocation(ctx.start, ctx.stop), ctx.IDENTIFIER().getText(), self.visit(ctx.identifier_list_option()), self.visit(ctx.block()))
    ...
    def visitTemplate_definition(self, ctx: CircomParser.Template_definitionContext):
        ...
        return Template(FileLocation(ctx.start, ctx.stop), ctx.IDENTIFIER().getText(), self.visit(ctx.identifier_list_option()), self.visit(ctx.block()), is_parallel, is_custom)
    ...
    def visitBlock(self, ctx: CircomParser.BlockContext):
        return Block(FileLocation(ctx.start, ctx.stop), self.visit(ctx.statement_list()))
    ...
    def visitExpression_statement(self, ctx: CircomParser.Expression_statementContext):
        return MultiSubstitution(FileLocation(ctx.start, ctx.stop), TupleExpr(FileLocation(ctx.start, ctx.stop), []), "<==", self.visit(ctx.expression()))
    ...
    def visitSubstitutions(self, ctx: CircomParser.SubstitutionsContext):
        if ctx.SELF_OP():
            ...
            return Substitution(FileLocation(ctx.start, ctx.stop), variable.name, variable.access, "=", InfixOp(FileLocation(ctx.start, ctx.stop), variable, op, Number(FileLocation(ctx.start, ctx.stop), 1)))
        elif ctx.ASSIGNMENT_WITH_OP():
            op = ctx.ASSIGNMENT_WITH_OP()[0]
            variable = self.visit(ctx.variable())
            return Substitution(FileLocation(ctx.start, ctx.stop), variable.name, variable.access, "=", InfixOp(FileLocation(ctx.start, ctx.stop), variable, op, self.visit(ctx.expression(0))))
        else:
            ...
            return Substitution(FileLocation(ctx.start, ctx.stop), var.name, var.access, op, self.visit(ctx.expression(1))) if isinstance(var, Variable) else MultiSubstitution(FileLocation(ctx.start, ctx.stop), var, op, self.visit(ctx.expression(1)))
    ...
    def visitIf_statement(self, ctx: CircomParser.If_statementContext):
        if ctx.ELSE():
            return IfThenElse(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression()), self.visit(ctx.regular_statement(0)), self.visit(ctx.if_statement()) if ctx.if_statement() else self.visit(ctx.regular_statement(1)))
        else:
            return IfThenElse(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression()), self.visit(ctx.if_statement()) if ctx.if_statement() else self.visit(ctx.regular_statement(0)), None)
    ...
    def visitFor_statement(self, ctx: CircomParser.For_statementContext):
        init = self.visit(ctx.declaration()) if ctx.declaration(
        ) else self.visit(ctx.substitutions(0))
        while_body = Block(FileLocation(ctx.start, ctx.stop), [self.visit(
            ctx.regular_statement()), self.visit(ctx.substitutions(1))])
        while_statement = While(FileLocation(
            ctx.start, ctx.stop), self.visit(ctx.expression()), while_body)
        return Block(FileLocation(ctx.start, ctx.stop), [init, while_statement])
    ...
    def visitWhile_statement(self, ctx: CircomParser.While_statementContext):
        return While(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression()), self.visit(ctx.regular_statement()))
    ...
    def visitEqual_constraint_statement(self, ctx: CircomParser.Equal_constraint_statementContext):
        return ConstraintEquality(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression(0)), self.visit(ctx.expression(1)))

    # return_statement: RETURN expression SEMICOLON;
    def visitReturn_statement(self, ctx: CircomParser.Return_statementContext):
        return Return(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression()))
    ...
    def visitAssert_statement(self, ctx: CircomParser.Assert_statementContext):
        return Assert(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression()))
    ...
    def visitLog_statement(self, ctx: CircomParser.Log_statementContext):
        return LogCall(FileLocation(ctx.start, ctx.stop), self.visit(ctx.log_list()) if ctx.log_list() else [])
    ...
    def visitVar_decl(self, ctx: CircomParser.Var_declContext):
        ...
        if ctx.some_symbol_list():
            ...
            for symbol in symbols:
                ...
                initializations.append(Declaration(FileLocation(
                    ctx.start, ctx.stop), Var(), name, dimensions, True))
                if init:
                    initializations.append(Substitution(FileLocation(
            ...
            for symbol in symbols:
                ...
                initializations.append(Declaration(FileLocation(
                    ctx.start, ctx.stop), Var(), name, dimensions, True))
                values.append(Variable(FileLocation(
                    ctx.start, ctx.stop), name, []))
            if ctx.tuple_initialization():
                op, expr = self.visit(ctx.tuple_initialization())
                initializations.append(MultiSubstitution(FileLocation(ctx.start, ctx.stop), TupleExpr(
                    FileLocation(ctx.start, ctx.stop), values), op, expr))
        return InitializationBlock(FileLocation(ctx.start, ctx.stop), Var(), initializations)
    ...
    def visitSignal_decl(self, ctx: CircomParser.Signal_declContext):
        ...
        if ctx.signal_symbol_list():
            ...
            if ctx.tuple_initialization():
                ...
                initializations.append(MultiSubstitution(FileLocation(ctx.start, ctx.stop), TupleExpr(
                    FileLocation(ctx.start, ctx.stop), values), op, expr))
        return InitializationBlock(FileLocation(ctx.start, ctx.stop), xtype, initializations)
    ...
    def visitSignal_header(self, ctx: CircomParser.Signal_headerContext):
        return Signal(FileLocation(ctx.start, ctx.stop), ctx.SIGNAL_TYPE().getText() if ctx.SIGNAL_TYPE() else "intermediate", self.visit(ctx.tag_list()) if ctx.tag_list() else [])
    ...
    def visitComponent_decl(self, ctx: CircomParser.Component_declContext):
        ...
        if ctx.some_symbol_list():
            ...
            if ctx.tuple_initialization():
                ...
                initializations.append(MultiSubstitution(FileLocation(ctx.start, ctx.stop), TupleExpr(
                    FileLocation(ctx.start, ctx.stop), values), op, expr))
        return InitializationBlock(FileLocation(ctx.start, ctx.stop), Component(), initializations)

    # var_access: array_access | component_access;
    def visitVar_access(self, ctx: CircomParser.Var_accessContext):
        return ArrayAccess(FileLocation(ctx.start, ctx.stop), self.visit(ctx.array_access())) if ctx.array_access() else ComponentAccess(FileLocation(ctx.start, ctx.stop), self.visit(ctx.component_access()))

    # var_access_list: var_access var_access_list | ;
    def visitVar_access_list(self, ctx: CircomParser.Var_access_listContext):
        return [self.visit(ctx.var_access())] + self.visit(ctx.var_access_list()) if ctx.var_access() else []
    ...
    def visitVariable(self, ctx: CircomParser.VariableContext):
        return Variable(FileLocation(ctx.start, ctx.stop), ctx.IDENTIFIER().getText(), self.visit(ctx.var_access_list()))
    ...
    def visitExpression(self, ctx: CircomParser.ExpressionContext):
        return ParallelOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression1())) if ctx.PARALLEL() else self.visit(ctx.expression1())
    ...
    def visitExpression1(self, ctx: CircomParser.Expression1Context):
        return InlineSwitchOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression2(1)), self.visit(ctx.expression2(0)), self.visit(ctx.expression2(2))) if ctx.TERNARY_ALTERNATIVE() else self.visit(ctx.expression2(0))
    ...
    def visitExpression2(self, ctx: CircomParser.Expression2Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression2()), ctx.OR().getText(), self.visit(ctx.expression3())) if ctx.OR() else self.visit(ctx.expression3())
    ...
    def visitExpression3(self, ctx: CircomParser.Expression3Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression3()), ctx.AND().getText(), self.visit(ctx.expression4())) if ctx.AND() else self.visit(ctx.expression4())
    ...
    def visitExpression4(self, ctx: CircomParser.Expression4Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression4()), self.visit(ctx.compareOpcode()), self.visit(ctx.expression5())) if ctx.compareOpcode() else self.visit(ctx.expression5())
    ...
    def visitExpression5(self, ctx: CircomParser.Expression5Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression5()), ctx.BOR().getText(), self.visit(ctx.expression6())) if ctx.BOR() else self.visit(ctx.expression6())
    ...
    def visitExpression6(self, ctx: CircomParser.Expression6Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression6()), ctx.BXOR().getText(), self.visit(ctx.expression7())) if ctx.BXOR() else self.visit(ctx.expression7())
    ...
    def visitExpression7(self, ctx: CircomParser.Expression7Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression7()), ctx.BAND().getText(), self.visit(ctx.expression8())) if ctx.BAND() else self.visit(ctx.expression8())
    ...
    def visitExpression8(self, ctx: CircomParser.Expression8Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression8()), self.visit(ctx.shiftOpcode()), self.visit(ctx.expression9())) if ctx.shiftOpcode() else self.visit(ctx.expression9())
    ...
    def visitExpression9(self, ctx: CircomParser.Expression9Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression9()), self.visit(ctx.add_sub_opcode()), self.visit(ctx.expression10())) if ctx.add_sub_opcode() else self.visit(ctx.expression10())
    ...
    def visitExpression10(self, ctx: CircomParser.Expression10Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression10()), self.visit(ctx.mul_div_opcode()), self.visit(ctx.expression11())) if ctx.mul_div_opcode() else self.visit(ctx.expression11())
    ...
    def visitExpression11(self, ctx: CircomParser.Expression11Context):
        return InfixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression11()), ctx.POW().getText(), self.visit(ctx.expression12())) if ctx.POW() else self.visit(ctx.expression12())
    ...
    def visitExpression12(self, ctx: CircomParser.Expression12Context):
        return PrefixOp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.prefixOpcode()), self.visit(ctx.expression12())) if ctx.prefixOpcode() else self.visit(ctx.expression13())
    ...
    def visitExpression13(self, ctx: CircomParser.Expression13Context):
        if ctx.listableAnon():
            ...
            for anon in listableAnon:
                ...
                names.append(anon[1])
            return AnonymousComponentExpr(FileLocation(ctx.start, ctx.stop), ctx.IDENTIFIER().getText(), False, self.visit(ctx.listable()), signals, names)
        elif ctx.getChildCount() == 4:
            return Call(FileLocation(ctx.start, ctx.stop), ctx.IDENTIFIER().getText(), self.visit(ctx.listable()))
        elif ctx.getChildCount() == 3:
            return ArrayInLine(FileLocation(ctx.start, ctx.stop), self.visit(ctx.listable())) if ctx.listable() else TupleExpr(FileLocation(ctx.start, ctx.stop), self.visit(ctx.twoElemsListable()))
        else:
    ...
    def visitExpression14(self, ctx: CircomParser.Expression14Context):
        if ctx.variable():
            ...
            return Variable(FileLocation(ctx.start, ctx.stop), ctx.UNDERSCORE().getText(), [])
        elif ctx.NUMBER():
            return Number(FileLocation(ctx.start, ctx.stop), int(ctx.NUMBER().getText(), 10))
        elif ctx.HEXNUMBER():
            return Number(FileLocation(ctx.start, ctx.stop), int(ctx.HEXNUMBER().getText(), 16))
        else:
            return self.visit(ctx.expression())
    ...
    def visitLog_arguement(self, ctx: CircomParser.Log_arguementContext):
        return LogExp(FileLocation(ctx.start, ctx.stop), self.visit(ctx.expression())) if ctx.expression() else LogStr(FileLocation(ctx.start, ctx.stop), ctx.STRING().getText())
    ...
    def visitListable(self, ctx: CircomParser.ListableContext):
        return [self.visit(ctx.expression())] + self.visit(ctx.listable()) if ctx.listable() else [self.visit(ctx.expression())]

    # listableAnon: listable | listableWithInputNames;
    def visitListableAnon(self, ctx: CircomParser.ListableAnonContext):
        return list(map(lambda x: (x, None), self.visit(ctx.listable()))) if ctx.listable() else self.visit(ctx.listableWithInputNames())
